...
void MessageManager_Send_FanStatus(FanStatus_t *s, uint32_t to_node)
{
	...
	data[4U] |= ((uint8_t) (s->DutyCycle >> 0U) << 0U) & 255U;
	
	data[5U] |= ((uint8_t) (s->Temperature >> 0U) << 0U) & 255U;
	
	CAN_NODE_0_SendMessage(CAN_MESSAGE_FANSTATUS_INDEX, (uint8_t *) data);
}
...
void MessageManager_HandleReceivedMessage(Message_t message)
{
	...
	if ((message.id & CAN_MESSAGE_FANCONTROL_MASK) == CAN_MESSAGE_FANCONTROL_MASK) {
		FanControl_t s;

		// Parse FanControl to struct

		s.DutyCycle = 0U;
		s.DutyCycle += ((message.data[0U]) & 255U);

		s.Direction = 0U;
		s.Direction += ((message.data[1U]) & 3U);

		s.Revolutions = 0U;
		s.Revolutions += ((message.data[2U]) & 255U);
		s.Revolutions += ((message.data[3U]) & 255U) << 8U;

		Handle_FanControl_Received(s, from_node_id, to_node_id);
	}
}
